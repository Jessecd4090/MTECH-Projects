...
struct NetworkController {
    ...
    func getComments(secret: UUID, postid: Int, pageNumber: Int? = nil) async throws -> [Comments] {
            // Initialize Components
        guard var components = URLComponents(string: "\(API.url)/comments") else {
            throw NetworkError.invalidURLComponents
        }
        
            // Set query items
        components.queryItems = [
            URLQueryItem(name: "userSecret", value: secret.uuidString),
    ...
    func editProfile(secret: UUID, userName: String, bio: String, techInterests: String) async throws {
